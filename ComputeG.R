...
#sink("D:....\\output_computeG.txt")
sink('outout_computeG.txt')
...
alpha = 1e-3
unrestricted = read.csv(file = 'D:....\\CogFuncAll.csv')
mysubs = read.table('D:....\\CogFuncAll.csv',sep=',', header = TRUE, fill = TRUE, quote="\"")
#View(mysubs)
...
#condition to do factor analysis
cortest.bartlett(cogdf)
KMO(cogdf)
...
out = fa.parallel(cogdf, plot=T, error.bars=F, se.bars=T, sim=F, fa="fa") #error.bars=T,se.bars=F,
...
dev.off()
summary(mod_biB, fit.measures=TRUE,standardized=TRUE)
cat("\n## fitMeasures\n")
